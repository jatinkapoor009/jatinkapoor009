# Hi, I'm Jatin Kapoor 👋

**DevOps Engineer** focused on cloud infrastructure, CI/CD automation, and DevSecOps practices.
I build real-world projects using AWS, Docker, Jenkins, and Terraform — turning manual processes into reliable, automated pipelines.


## 🛠️ Tech Stack

**Cloud**
![AWS](https://img.shields.io/badge/AWS-232F3E?style=flat&logo=amazonaws&logoColor=white)
![EC2](https://img.shields.io/badge/EC2-FF9900?style=flat&logo=amazonec2&logoColor=white)
![S3](https://img.shields.io/badge/S3-569A31?style=flat&logo=amazons3&logoColor=white)
![IAM](https://img.shields.io/badge/IAM-DD344C?style=flat&logo=amazonaws&logoColor=white)
![VPC](https://img.shields.io/badge/VPC-8C4FFF?style=flat&logo=amazonaws&logoColor=white)
![CloudWatch](https://img.shields.io/badge/CloudWatch-FF4F8B?style=flat&logo=amazonaws&logoColor=white)

**CI/CD & Containers**
![Jenkins](https://img.shields.io/badge/Jenkins-D24939?style=flat&logo=jenkins&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat&logo=docker&logoColor=white)
![GitHub Actions](https://img.shields.io/badge/GitHub_Actions-2088FF?style=flat&logo=githubactions&logoColor=white)

**IaC & Security**
![Terraform](https://img.shields.io/badge/Terraform-7B42BC?style=flat&logo=terraform&logoColor=white)
![Trivy](https://img.shields.io/badge/Trivy-1904DA?style=flat&logo=aquasecurity&logoColor=white)
![SonarQube](https://img.shields.io/badge/SonarQube-4E9BCD?style=flat&logo=sonarqube&logoColor=white)

**Scripting & OS**
![Bash](https://img.shields.io/badge/Bash-4EAA25?style=flat&logo=gnubash&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=flat&logo=linux&logoColor=black)
![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)


## 🚀 Featured Projects

### 🔷 [AutoDeployX — DevSecOps CI/CD Pipeline](https://github.com/jatinkapoor009/autodeployx_static)
End-to-end automated pipeline with security scanning built in.
- GitHub Webhook → Jenkins → Docker Build → **Trivy Scan** → Docker Compose Deploy → AWS EC2
- CloudWatch alarms configured for real-time uptime monitoring

### 🔷 [Cloud Infrastructure Automation — Terraform & AWS](https://github.com/jatinkapoor009/3-tier-archiecture_code)
Full AWS stack provisioned as code — no manual console clicks.
- EC2, IAM, VPC, ELB provisioned via reusable Terraform modules
- Shell scripts for health checks; CloudWatch alarms across 5 components
- Reduced provisioning time from **2 hours → under 10 minutes**

### 🔷 [3-Tier Architecture on AWS](https://github.com/jatinkapoor009/3-tier-archiecture)
Production-style 3-tier deployment (Web → App → DB) on AWS with proper networking.

### 🔷 [CI/CD Pipeline — Java Maven + Docker](https://github.com/jatinkapoor009/CI-CD-java-maven-docker)
Automated build and containerization pipeline for a Java Maven application using Jenkins and Docker.


## 📊 GitHub Stats

<p align="left">
  <img src="https://github-readme-stats.vercel.app/api?username=jatinkapoor009&show_icons=true&theme=default&hide_border=true&count_private=true" height="150"/>
  &nbsp;
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=jatinkapoor009&layout=compact&theme=default&hide_border=true" height="150"/>
</p>



## 📬 Connect

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=flat&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/jatin-kapoor1/)
[![Portfolio](https://img.shields.io/badge/Portfolio-000000?style=flat&logo=githubpages&logoColor=white)](https://jatinkapoor009.github.io)
[![Email](https://img.shields.io/badge/Email-EA4335?style=flat&logo=gmail&logoColor=white)](mailto:Jatink9599@gmail.com)



*"Automate everything. Monitor everything. Ship with confidence."*
